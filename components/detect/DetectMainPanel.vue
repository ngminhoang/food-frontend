...
const analyzeImage = async () => {
  ...
  try {
    ...
    // Check if calories in the analysis data and sumCalories from the store are non-zero
    const caloriesFromApi = analysisData.value.items[0].food[0].food_info.nutrition.calories_100g;
    const sumCalories = nutrientStore.nutrients.reduce((acc, nutrient) => acc + nutrient.nuCalories, 0);

    if (caloriesFromApi !== 0 && sumCalories !== 0) {
      console.log('Nutrient store data:', nutrientStore.nutrients);
    }
